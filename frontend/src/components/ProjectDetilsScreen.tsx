...
const ProjectDetails = () => {
  ...
  return (
    <div>
      ...
      <p>Assigned Users: {projectData.users.map(user => `${user.name} ${user.surname}`).join(', ')}</p>
      ...
      <table>
        ...
        <tbody>
          {comments.map((comment) => (
            <tr key={comment.id} style={comment.author === projectData.author ? { backgroundColor: 'lightblue' } : {}}>
